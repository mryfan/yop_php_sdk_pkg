...
namespace Fy\Yeepay\Yop\Sdk\Http;


use Fy\GuzzleHttp\Client;
use Fy\GuzzleHttp\Exception\GuzzleException;
use Fy\GuzzleHttp\Exception\ServerException;
use Fy\GuzzleHttp\RequestOptions;
use Fy\Psr\Http\Message\ResponseInterface;
use Fy\Psr\Log\LoggerInterface;
use Fy\Yeepay\Yop\Sdk\Exception\YopClientException;
use Fy\Yeepay\Yop\Sdk\Internal\MultiPartFile;
use Fy\Yeepay\Yop\Sdk\Internal\Request;
use Fy\Yeepay\Yop\Sdk\Log\LogFactory;
use Fy\Yeepay\Yop\Sdk\Utils\Http\HttpUtils;
...
class YopHttpClient
{
    ...
    private function sendRequest(Request $request)
    {
        ...
        if ($putParamsInUri) {
            $encodedParameters = \Fy\GuzzleHttp\Psr7\build_query(HttpUtils::encodedParameters($request->getParameters()));
        ...
        if ($requestHasPayload) {
            ...
            $requestOptions[RequestOptions::BODY] = \Fy\GuzzleHttp\Psr7\build_query(HttpUtils::encodedParameters($request->getParameters()));
        }
